...
- **解放後に `Instance` を触ると空のインスタンスが復活する**: `Instance` は `instance ?? (instance = CreateInstance())` の遅延生成。`ReleaseInstance()` 後に演出解除・ボタン操作・画面タップ等から `Instance` を参照すると、初期化されていないインスタンスが黙って生成される。さらに `CreateInstance()` は `instance != null` なら既存をそのまま返すため、**次のフェーズで `CreateInstance()` しても残骸が再利用され、状態が持ち越される**。`var m = Xxx.Instance; if (m == null){ return; }` というガードは `Instance` 経由では常に非 null になるため**機能しない**。解放後も呼ばれ得る箇所では必ず `if (!Xxx.Exists){ return; }` で生成済みか判定してから `Instance` を参照すること。
- **一括解放してから一括生成し直す形の対策は取らないこと**: 「毎回まっさらから始める」目的で `CreateInstance()` の前に `ReleaseInstance()` を挟むと、**先に `Instance` を取得して Subject を購読済みのコードが購読先を失う**（購読は解放された旧インスタンスに残り、新インスタンス側は Subject が null なので通知が届かない）。生成より先に購読が走る設計（シーンの Initialize で購読 → Prepare でマネージャー生成 等）では確実に壊れる。状態の持ち越しは「解放を確実に行う」+「解放後は `Exists` 判定」で防ぐ。
