...
- **解放後に `Instance` を触ると空のインスタンスが復活する**: `Instance` は `instance ?? (instance = CreateInstance())` の遅延生成。`ReleaseInstance()` 後に Update 等から `Instance` を参照すると、初期化されていないインスタンスが黙って生成される。さらに `CreateInstance()` は `instance != null` なら既存をそのまま返すため、**次のフェーズで `CreateInstance()` しても残骸が再利用され、状態が持ち越される**。`if (manager == null)` のガードは `Instance` 経由では常に非 null になるため機能しない（判定するなら `Exists` を使う）。対策は次の2点をセットで守ること: (1) 解放は参照元（Update を回すマネージャー・View 等）が生きている間は行わず、それらの破棄後に行う (2) フェーズ開始時の一括生成では生成前に一括解放を挟み、必ず新しいインスタンスから始める。
