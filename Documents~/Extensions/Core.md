...
| フォルダ | 主要クラス |
|---|---|
| `Behaviour/` | `Singleton<T>`（非MonoBehaviourシングルトン基底。Client の各種 Manager の基底） / `SingletonMonoBehaviour<T>`（シーン上コンポーネントのシングルトン基底。`[ExecuteAlways]` 付き） / `SingletonManager`（全インスタンスの登録簿。一括 Refresh / ReleaseAll） / `ISingleton`（`Refresh()` / `Release()` の共通IF） / `LifetimeDisposable`（R3 `CompositeDisposable` を保持し Dispose で購読を一括解除） |
...
- `Singleton<T>` マネージャー定義（`OnCreate()` で購読登録 + `.AddTo(Disposable)`。利用側はどこからでも `Instance`）: `Client/Assets/Scripts/Client/Core/Sound/BgmManager.cs`
- `CreateInstance()` / `ReleaseInstance()` による明示的な生成・破棄管理（戦闘開始時に戦闘系シングルトンを一括生成、終了時に一括解放）: `Client/Assets/Scripts/Client/Battle/Core/Manager/BattleManager.cs`
- `SingletonMonoBehaviour<T>` + `Prefab` フィールド: `Client/Assets/Scripts/Client/Core/LoadingIconManager.cs`
- シーン上に存在しない場合のフォールバック生成（`Instance ?? UnityUtility.Instantiate<T>(null, prefab)`）: `Client/Assets/Scripts/Client/Core/Initialize/InitializeObject/InitializeObject.core.cs`
- `Prefab` によるリストアイテム複数生成（`prefab.Instantiate<T>(count)`）: `Client/Assets/Scripts/Client/Feature/Window/Langage/LangageSelectWindow.cs`
- `Prefab` + ObjectPool 併用（`Source` / `Parent` に分解して渡す）: `Client/Assets/Scripts/Client/Scene/Battle/View/BattleLog/BattleLogView.cs`
- `[SerializeField, ReadOnly]` 表示専用フィールド: `Client/Assets/Scripts/Client/Core/AppConfig.cs`
- `[Label("表示名")]` enum + `ToLabelName()`: `Client/Assets/UniModules/Scripts/Modules/Network/WebRequest/WebRequestManager.cs`
- SafeValue（`XInt`）で戦闘のバフ効果時間をメモリ改ざん対策付きで保持: `Client/Assets/Scripts/Client/Battle/Core/Status/BattleStatus.cs`
- `FloatNullable` のインスペクタ利用（Client側は未使用・基盤内実例）: `Client/Assets/UniModules/Scripts/Modules/UI/Layout/PreferredSizeCopy.cs`
- `MathematicsRandomUtility` の戦闘ロジック使用例: `Client/Assets/Scripts/Client/Battle/Brain/BrainBase.cs`
