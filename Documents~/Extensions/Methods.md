...
| Dictionary から安全に値取得 | `dict.GetValueOrDefault(key)` |
...
| `ToHashSet([comparer])` | `#if !UNITY_2021_2_OR_NEWER` 限定。Unity 2021.2 以降では BCL 版が使われる |

### Dictionary（DictionaryExtensions.cs）

| メソッド | 説明 |
|---|---|
| `GetValueOrDefault(key, defaultValue = default)` | キーが無ければ既定値。`#if !UNITY_2021_2_OR_NEWER` 限定で、Unity 2021.2 以降では BCL 版が解決される（挙動同等） |
...
- `DictionaryExtensions.GetValueOrDefault` / `EnumerableExtensions.ToHashSet` は `#if !UNITY_2021_2_OR_NEWER` 内。Unity 2021.2 以降では**コンパイル対象外**で、呼び出しは BCL 版に解決される（挙動は同等なのでコードはそのまま書いてよい）。
...
- [TextData.md](../Modules/TextData.md) — テキスト取得
