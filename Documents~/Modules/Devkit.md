...
非メインスレッドからも呼べる（`SynchronizationContext.Post` でメインへ）。機能専用ログクラスの定型（イベント名+色を定義して `UnityConsole.Event` を呼ぶ）: `Client/Assets/Scripts/Client/Battle/Develop/BattleLog.cs` 参照。

## 使い方

- **Diagnosis（実機デバッグ表示）**: `Diagnosis` prefab（FpsStats / MemoryStats / SRDiagnosis を内包）を `InitializeObject.devkit.cs` が生成・`Initialize()` する（`ENABLE_DEVKIT || ((UNITY_EDITOR || DEVELOPMENT) && !PRODUCTION)` 時のみコンパイル）。`SRDiagnosis.IgnoreWarnings / IgnoreErrors`（プレフィックス一致で無視）の設定例も同ファイル: `Client/Assets/Scripts/Client/Core/Initialize/InitializeObject/InitializeObject.devkit.cs`
- **SendReport（不具合レポート送信）**: `SendReportManager.Instance.Initialize(uploader)`（必須・1回）→ `Send(reportTitle)` で SS+直近ログ（`UnityLogTracker`）+任意項目を送信。追加項目は `OnRequestReportAsObservable()` 購読内で `AddReportContent(key, value)`、`SetCryptKey` で値をAES暗号化。BugLogServer への設定実例: `Client/Assets/Scripts/Client/Devkit/SRDebugger/SendReportController.cs`（SRDebuggerタブUIの基底は `SendReportSheetController`）
- **ログ購読**: `ApplicationLogHandler.Instance.OnReceivedAllAsObservable()` 等（種別別 / Threaded 版あり。Threaded 版は `ObserveOn(UnityFrameProvider.Update)` と併用が定石）。`DebugLog` は `Receive*` → Observable 配信の static ハブで、Client では Bugsnag へのエラー転送に使用。実例: `Client/Assets/Scripts/Client/Core/Initialize/InitializeObject/InitializeObject.core.cs`
- **エディタ拡張の登録パターン**: 基盤が枠を提供し、Client側が `[InitializeOnLoadMethod]` で登録する構造が共通（登録ファイルはカタログ表の各行参照: `AssetTunerRegister.cs` / `HierarchyItemDrawer.cs` / `InspectorRegister.cs` / `ProjectFileHook.cs`）。ビルド前処理で Devkit 各機能を組み合わせる実例: `Client/Assets/Scripts/Editor/Build/ApplicationBuilder.cs`（`OnBeforeBuild` で `FixSpriteAtlasSource.Modify` / `PrefabDummyTextCleaner.CleanAllPrefabContents` / `MemoComponentRemover.RemoveAll*` を実行）
...
- メニュー拡張時は Client側 `Dominion.Editor.EditorMenu`（`Modules.EditorMenu` 継承）または `ProductEditorMenu` に追加する（基盤側 EditorMenu.cs は直接編集しない）。メニュー登録の実体: 基盤 `Client/Assets/UniModules/Scripts/Editor/EditorMenu.cs` を Client側 `Client/Assets/Scripts/Editor/EditorMenu.cs` が継承し、`ProductEditorMenu.cs` が `Dominion/...` を追加
