...
- **ロード基本形（最頻出。パス直指定・型指定）**: `var atlas = await ExternalAsset.LoadAsset<SpriteAtlas>("Contents/Item/Icon/ItemIconAtlas.spriteatlasv2");` — 引用元: `Client/Assets/Scripts/Client/Manager/CommonAssetManager.cs`（`SpriteAtlasCache` と併用）、TextAsset系: `Client/Assets/Scripts/Client/Module/Download/ContentsUpdateManager.cs`
- **autoUnload: false + 明示解放**: `LoadAsset<SpriteAtlas>(loadPath, false)` → Sprite取得後に `ExternalAsset.UnloadAssetBundle(loadPath)` — 引用元: `Client/Assets/Scripts/Client/Tutorial/TutorialNavigation.cs`
- **事前ダウンロード（進捗付き並列DL）**: `GetGroupAssetInfos(group)` → `IsRequireUpdate` でDLが必要なアセットのみに絞り込み → 各 `ExternalAsset.UpdateAsset(resourcePath, progress)` を `UniTask.WhenAll`（進捗は `IProgress<DownloadProgressInfo>` で受けて合算）— 引用元: `Client/Assets/Scripts/Client/Scene/WorldMap/TileMap/WorldMapTileManager.cs`
- **起動時の一括更新**: `GetRequireUpdateAssetInfos()`（全グループの要更新一覧）→ グループフィルタ後に順次 `ExternalAsset.UpdateAsset(assetInfo.ResourcePath, cancelToken: cancelToken)` — 引用元: `Client/Assets/Scripts/Client/Module/Download/ContentsUpdateManager.cs`
- **初期化（起動時1回。通常は触らない — InitializeObject が実施済み）**: `Initialize(externalAssetDirectory, shareAssetDirectory)` → `SetInstallDirectory(UnityPathUtility.PersistentDataPath)` → `SetAssetBundleFileStreamFactory`（AES-CTR復号）→ `OnErrorAsObservable` / `OnTimeOutAsObservable` 購読（エラー時は共通ハンドリングでタイトルへ遷移）— 引用元: `Client/Assets/Scripts/Client/Core/Initialize/InitializeObject/InitializeObject.manager.cs` の `InitializeExternalAsset`
- **起動フロー: URL設定 → マニフェスト更新（タイトル画面）**: `SetUrl(Urls.AssetsUrl, rootHash)`（rootHash は PlayFab TitleData (AssetRootHash) 由来）→ `UpdateManifest()`（マスター更新と `UniTask.WhenAll` で並列実行）— 引用元: `Client/Assets/Scripts/Client/Module/Download/ContentsUpdateManager.cs` の `FetchInitalData`
...
- **LoadAsset は throw しない**: 失敗時は null を返し `OnErrorAsObservable` に流れる（Clientの購読でタイトル画面へ強制遷移する）。呼び出し側での null チェックは「エラー時に処理を進めない」ためのガードとして書く。`T : UnityEngine.Object` で、Component 派生型を指定すると GameObject 経由で GetComponent される。
...
- **更新判定はファイル存在のみ**: キャッシュ名 = `Hash先頭32文字+拡張子` なので中身のハッシュ再計算はしない。壊れたキャッシュはロード失敗時に `DeleteOnLoadError = true` で自動削除→次回再DL。不要キャッシュは `UpdateManifest` 後に1日1回自動削除（`Prefs.deleteUnUsedCacheTime`）。`DeleteAllCache` は削除前に `OnReleaseManagedAssetsAsObservable` が発火する（Client はここで SpriteAtlasCache 等を解放）。
...
- **`ShaderReApply`** は例外的に `Start()` を使う既存MonoBehaviour（AssetBundle由来マテリアルのシェーダー再適用。エディタでのピンク化対策）。新規コードの参考にしない。
