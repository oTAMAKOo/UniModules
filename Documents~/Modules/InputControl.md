...
主要クラス: `BlockInput`（生成でロック・Dispose で解除する Scope。基本の入口） / `BlockInputManager`（Singleton。ブロックID集合の管理・状態変化通知・例外時の強制解除） / `InputBlockListener`（常駐全画面キャンバスの表示切替 MonoBehaviour） / Client側 `LoadingScope`（`BlockInput` + `LoadingIconManager.Show/Hide` の複合スコープ） / `BlockInputMonitorWindow`（エディタ専用デバッグウィンドウ）。
...
- **全体リセット時の強制解除**（通常フローでは呼ばない）: `BlockInputManager.Instance.ForceUnlock()`（実例: `Client/Assets/Scripts/Client/Model/System/SystemModel.cs` のタイトルへ戻す処理）
...
- ブロックの実体は起動時に `InitializeObject.CreateScreenInputBlock()`（`Client/Assets/Scripts/Client/Core/Initialize/InitializeObject.core.cs`）が生成する常駐プレハブ `Client/Assets/Resource (Internal)/Core/Prefabs/Canvas (InputBlock).prefab`（DontDestroyOnLoad）。**Client実装側での生成・配置・`InputBlockListener` の個別アタッチは不要**
