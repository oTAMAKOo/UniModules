...
- **リストアイテムのプール**（Initialize で `new ObjectPool` + `Resize` で prewarm、Setup 冒頭で表示中アイテムを全 `Release` してから再構築）: `Client/Assets/Scripts/Client/Scene/Citadel/Window/FacilityLevelWindow/FacilityLevelListView.cs`
- **スタイル別複数プール + 生成時初期化**（`Dictionary<Style, ObjectPool<T>>` で複数プールを構築し、`OnCreateInstanceAsObservable` で新規インスタンスにだけ初期設定、Get 直後に自前リセット・Release 前に子要素を先に返却）: `Client/Assets/Scripts/Client/Scene/Battle/Vfx/PopText/Manager/PopTextEntryManager.cs`
