...
全ファイルが `#if UNITY_STANDALONE_WIN` で囲まれており、**モバイルビルド・他プラットフォームではコンパイル対象外**（クラス自体が存在しない）。
...
- `#if UNITY_STANDALONE_WIN` 専用。**他プラットフォーム（モバイル等）では参照するとコンパイルエラー**になるため、使う場合は呼び出し側も同シンボルで囲む
